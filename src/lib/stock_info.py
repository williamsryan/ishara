...
def get_quote_table(ticker, dict_result=True, headers={'User-agent': 'Mozilla/5.0'}):
    ...
    Args:
        ...
        dict_result (bool): Whether to return the result as a dictionary.
...
def _parse_json(url, headers={'User-agent': 'Mozilla/5.0'}):
    """
    Parses JSON data from a Yahoo Finance URL.
    ...
    try:
        ...
        json_data = re.search(r'root\.App\.main = (.+?);\n', html_content).group(1)
        data = json.loads(json_data)["context"]["dispatcher"]["stores"]["QuoteSummaryStore"]
        
        return data
    except Exception as e:
        print(f"⚠️ Failed to parse JSON from {url}: {e}")
        return {}
...
def get_live_price(ticker):
    """
    Gets the live price of the input ticker.
